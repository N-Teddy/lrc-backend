...
async function bootstrap() {
  ...
  process.env.APP_CODE = AppCode.CENTRE;

  const logger = new Logger('CentreBootstrap');
  const app = await NestFactory.create<NestExpressApplication>(CentreModule);

  const configService = app.get(ConfigService);
  const port = configService.get<number>('CENTRE_PORT', 3004);

  app.use('/uploads', express.static(join(process.cwd(), 'uploads')));

  setupApp(app, 'Centre', 'saturn');

  await app.listen(port);

  logger.log(`Centre Service is running on: http://localhost:${port}/api`);
