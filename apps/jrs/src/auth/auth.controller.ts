...
import { JrsProvisionUserDto } from './dto/jrs-provision-user.dto';
import { JrsResendInviteDto } from './dto/jrs-resend-invite.dto';
import { JrsAcceptInviteDto } from './dto/jrs-accept-invite.dto';
...
export class AuthController {
  ...
  @Public()
  @Post('login')
  @ApiOperation({ summary: 'Login with credentials for JRS access' })
  @ApiResponse({
    status: 201,
  ...
  @UseGuards(JwtAuthGuard)
  @Post('provision')
  @ApiOperation({ summary: 'Provision a new user for JRS app' })
  @ApiResponse({ status: 201, description: 'User provisioned successfully' })
  @ApiResponse({ status: 400, description: 'Validation failed' })
  @ApiResponse({ status: 404, description: 'Person not found' })
  @ApiBody({ type: JrsProvisionUserDto })
  provision(@Body() dto: JrsProvisionUserDto): Promise<unknown> {
  ...
  @ApiBody({ type: JrsResendInviteDto })
  resendInvite(@Body() dto: JrsResendInviteDto): Promise<unknown> {
  ...
  @ApiBody({ type: JrsAcceptInviteDto })
  acceptInvite(@Body() dto: JrsAcceptInviteDto): Promise<unknown> {
